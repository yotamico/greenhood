...
- Only a community-confirmed closure opens a 3h dispute/appeal window (`/api/items/[id]/dispute`) — 2 "still there?" votes reopen the item. `app/map/page.tsx`/`app/feed/page.tsx` keep showing `status==='taken'` items during this window (dimmed pin / "⏳ סומן כנלקח" pill) so nearby users can actually see and dispute them. Owner-direct closures skip this window and drop off the map/feed immediately — gated by `closed_by`, which is set only by `/api/items/[id]/confirm-taken` (never by `updateStatus`) and cleared on reopen/re-close, so it always reflects whether the *current* closure was community-confirmed.
